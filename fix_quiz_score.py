...
# Replace the incorrect update_score call
patch = r'''
    total_points = None
...
text = re.sub(r'    total_points = None\n    if gained and not already:\n        try:\n            await update_score\(req.game_code, req.player_id, gained\)\n            g = await get_game\(req.game_code\)\n            if g:\n                p = next\(\(pl for pl in g.get\("players", \[\]\) if pl.get\("id"\) == req.player_id\), None\)\n                if p:\n                    total_points = p.get\("score", p.get\("points"\)\)\n        except Exception:\n            pass', patch.strip(), text, flags=re.DOTALL)

with open(r'c:\Users\PC\Downloads\TIENDA_UAA_v9\TIENDA UAA\backend\api\quiz.py', 'w', encoding='utf-8') as f:
    f.write(text)

print("Fixed quiz.py score updating!")
