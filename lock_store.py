...
# Replace renderMissions with logic to lock the store mission
render_missions_patch = r"""
    function renderMissions(missions) {
      ...
      missions.forEach((m, i) => {
        ...
        let clickAction = isQuiz ? `window.location.href='mision.html?mission=${m.mission_id}'` : `window.location.href='game.html?mission=${m.mission_id}'`;
        ...
        if (isStore) {
          if (!allQuizzesCompleted) {
             ...
             statusClass = 'status-completed'; // Uses gray styling
             btnText = '🔒 BLOQUEADA';
             btnClass = 'btn-done';
        ...
        if (isQuiz) {
          card.innerHTML = `
            ...
            <button class="btn-mission ${btnClass}" onclick="${clickAction}">${btnText}</button>
          `;
        } else {
          const meta = MISSION_META[m.mission_type] || { icon:'🛒', diff:'ESPECIAL', diffClass:'diff-media', pts:'+150 pts', time:'Libre', desc: 'Resuelve el problema en la tienda.', topic: 'MISIÓN FINAL' };
          card.innerHTML = `
            <div class="card-top">
              <div style="flex:1">
...
print("Store mission lock applied!")
